--stg_calendar has 50 unique listing_id, stg_listings has 49 unique, stg_amenities_changelog has 50--
--making sure that amenities in stg_listings are actually up-to-date--

{{config(materialized='view')}}

with stg_amenities_changelog as (
    select * from {{ref('stg_amenities_changelog')}}
)

, stg_listings as (
    select * from {{ref('stg_listings')}}
)

, amenities_update as (
    select *
    , row_number() over (partition by listing_id order by amenities_change_date desc) as row_number
    from stg_amenities_changelog
)

, current_amenities_final as (
    select listing_id
    , amenities_change_date
    , amenities
    from amenities_update 
    where row_number = 1)

, final as (
    select l.listing_id, l.neighborhood, c.amenities
    from stg_listings l
    inner join current_amenities_final c on l.listing_id = c.listing_id)

select * from final